...
(0, globals_1.describe)("POST /users/me/money", () => {
    (0, globals_1.describe)("succès", () => {
        (0, globals_1.it)("devrait retourner 200 avec le nouveau solde pour pack-50", async () => {
            globals_1.jest.mocked(profileService.addMoney).mockResolvedValue({
                ok: true,
                value: 150,
            });
            ...
            (0, globals_1.expect)(res.body.money).toBe(150);
        });
        (0, globals_1.it)("devrait retourner 200 avec le nouveau solde pour pack-200", async () => {
            globals_1.jest.mocked(profileService.addMoney).mockResolvedValue({
                ok: true,
                value: 300,
            });
            ...
            (0, globals_1.expect)(res.status).toBe(200);
            (0, globals_1.expect)(res.body.money).toBe(300);
        });
        (0, globals_1.it)("devrait appeler addMoney avec le bon userId et le bon montant", async () => {
            globals_1.jest.mocked(profileService.addMoney).mockResolvedValue({
                ok: true,
                value: 150,
            });
            await (0, supertest_1.default)(app)
                .post("/users/me/money")
                .send({ packId: "pack-50" });
            (0, globals_1.expect)(profileService.addMoney).toHaveBeenCalledWith("user-id-123", 50);
        });
        (0, globals_1.it)("devrait appeler addMoney avec le montant correct pour chaque pack", async () => {
            globals_1.jest.mocked(profileService.addMoney).mockResolvedValue({
                ok: true,
                value: 1100,
            });
            ...
            (0, globals_1.expect)(profileService.addMoney).toHaveBeenCalledWith("user-id-123", 1000);
        });
    });
    ...
    (0, globals_1.describe)("validation des inputs", () => {
        (0, globals_1.it)("devrait retourner 400 si packId est manquant", async () => {
            const res = await (0, supertest_1.default)(app)
                .post("/users/me/money")
                .send({});
            (0, globals_1.expect)(res.status).toBe(400);
            (0, globals_1.expect)(res.body.error).toBe("INVALID_PACK");
        });
        (0, globals_1.it)("devrait retourner 400 si packId est invalide", async () => {
            const res = await (0, supertest_1.default)(app)
                .post("/users/me/money")
        ...
        (0, globals_1.it)("devrait retourner 400 si packId est une chaîne vide", async () => {
            const res = await (0, supertest_1.default)(app)
                .post("/users/me/money")
                .send({ packId: "" });
            (0, globals_1.expect)(res.status).toBe(400);
...
//# sourceMappingURL=post.money.route.test.js.map